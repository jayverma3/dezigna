...
const ListingDetail = () => {
  ...
  useEffect(() => {
    ...
    if (foundListing) {
      ...
      const gallery =
        Array.isArray(foundListing.gallery_images) &&
        foundListing.gallery_images.length > 0
      ...
      const initialIndex = gallery.findIndex(
        (img) => img === (foundListing.image || gallery[0])
      );
  ...
  const galleryImages =
    Array.isArray(listing.gallery_images) && listing.gallery_images.length > 0
    ...
    <>
      ...
      <div className="ListingDetail">
        ...
        >
          {/* Image Gallery */}
          <motion.div
            className="ListingDetail__gallery"
          ...
          >
            <AnimatePresence mode="wait">
              ...
              >
                <ImageZoom src={mainImage} alt={listing.title} />
                <button
                  onClick={() => handleNavClick("prev")}
                  className="ListingDetail__navArrow ListingDetail__navArrow--prev"
                >
                  <ChevronLeft size={24} />
                </button>
                <button
                  onClick={() => handleNavClick("next")}
                  className="ListingDetail__navArrow ListingDetail__navArrow--next"
                >
            ...
            <div className="ListingDetail__thumbnailReel">
              {galleryImages.map((image, index) => (
                <motion.div
                  key={index}
                  className={`ListingDetail__thumbnailContainer ${
                    currentIndex === index
                      ? "ListingDetail__thumbnailContainer--active"
                      : ""
                  }`}
          ...
          {/* Product Info */}
          <motion.div
            className="ListingDetail__infoContainer"
          ...
          >
            ...
            <motion.div
              variants={infoVariants}
              className="ListingDetail__section"
            >
              ...
              <ul className="ListingDetail__featuresList">
                {listing.features.map((feature, index) => (
                  <li key={index} className="ListingDetail__featureItem">
                    <CheckCircle
                      className="ListingDetail__featureIcon"
                      size={18}
                    />
                    <span>{feature}</span>
                  </li>
                  ...
                  {Object.entries(listing.specifications).map(
                    ([key, value]) => (
                      <div key={key} className="ListingDetail__specItem">
                        ...
                        <span className="ListingDetail__specValue">
                          {value}
                        </span>
                      </div>
                    )
            ...
            {listing.materials && listing.materials.length > 0 && (
              <motion.div
                ...
                className="ListingDetail__section"
              >
                <h3 className="ListingDetail__sectionTitle">
                  Available Materials
                </h3>
                <div className="ListingDetail__materialsTags">
                  {listing.materials.map((material, index) => (
                    <span key={index} className="ListingDetail__materialTag">
                      {material}
                    </span>
                  ))}
                </div>
            ...
            <motion.div
              variants={infoVariants}
              className="ListingDetail__ctaSection"
            >
              <motion.button
              ...
              <p className="ListingDetail__quoteSubtext">
